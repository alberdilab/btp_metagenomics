...
Temperate_woodland_samples <- sample_metadata %>% 
  filter(broad_environment=="Temperate woodland") %>% 
  dplyr::select(sample) %>% 
  pull()
...
```{r phyla_cap_xeric, comment="", echo=FALSE, message=FALSE, warning=FALSE, eval = FALSE}

xeric_shrubland_samples <- sample_metadata %>% 
  filter(broad_environment=="Xeric shrubland") %>% 
  dplyr::select(sample) %>% 
  pull()
...
mixed_forest_samples <- sample_metadata %>% 
  filter(broad_environment=="Mixed forest") %>% 
  dplyr::select(sample) %>% 
  pull()
...
cropland_samples <- sample_metadata %>% 
  filter(broad_environment=="Cropland") %>% 
  dplyr::select(sample) %>% 
  pull()
...
phylum relativ abundance - broad environment clusters
...
phylum_summary %>%
  ...
  summarise(
    total_mean = mean(relabun * 100, na.rm = TRUE),
    total_sd = sd(relabun * 100, na.rm = TRUE),
    le_mean = mean(ifelse(broad_environment == "Cropland", relabun * 100, NA), na.rm = TRUE),
    le_sd = sd(ifelse(broad_environment == "Cropland", relabun * 100, NA), na.rm = TRUE),
    ha_mean = mean(ifelse(broad_environment == "Mixed forest", relabun * 100, NA), na.rm = TRUE),
    ha_sd = sd(ifelse(broad_environment == "Mixed forest", relabun * 100, NA), na.rm = TRUE),
    er_mean = mean(ifelse(broad_environment == "Xeric shrubland", relabun * 100, NA), na.rm = TRUE),
    er_sd = sd(ifelse(broad_environment == "Xeric shrubland", relabun * 100, NA), na.rm = TRUE),
    go_mean = mean(ifelse(broad_environment == "Temperate woodland", relabun * 100, NA), na.rm = TRUE),
    go_sd = sd(ifelse(broad_environment == "Temperate woodland", relabun * 100, NA), na.rm = TRUE)
  ) %>%
  mutate(
    Total = str_c(round(total_mean, 3), "±", round(total_sd, 3)),
...
family_summary %>%
    ...
    summarise(total_mean=mean(relabun*100, na.rm=T),
              ...
              le_mean=mean(relabun[broad_environment=="Cropland"]*100, na.rm=T),
             le_sd=sd(relabun[broad_environment=="Cropland"]*100, na.rm=T),
              ha_mean=mean(relabun[broad_environment=="Mixed forest"]*100, na.rm=T),
              ha_sd=sd(relabun[broad_environment=="Mixed forest"]*100, na.rm=T),
              er_mean=mean(relabun[broad_environment=="Xeric shrubland"]*100, na.rm=T),
              er_sd=sd(relabun[broad_environment=="Xeric shrubland"]*100, na.rm=T),
              go_mean=mean(relabun[broad_environment=="Temperate woodland"]*100, na.rm=T),
              go_sd=sd(relabun[broad_environment=="Temperate woodland"]*100, na.rm=T)) %>%
    mutate(Total=str_c(round(total_mean,3),"±",round(total_sd,3)),
          Cropland=str_c(round(le_mean,3),"±",round(le_sd,3)),
          Mixed_forest=str_c(round(ha_mean,3),"±",round(ha_sd,3)),
          Xeric_shrubland=str_c(round(er_mean,3),"±",round(er_sd,3)),
...
*** Percetange of genera in each group of broad environment clusters***
...
```{r phyloseq, comment="", echo=FALSE, message=FALSE, warning=FALSE, eval = FALSE}
#Phyloseq object
...
```{r struct_zero, comment="", echo=FALSE, message=FALSE, warning=FALSE, eval = FALSE}
...
```{r zero_phylo, comment="", echo=FALSE, message=FALSE, warning=FALSE, eval = FALSE}
...
domain_gift_filt <- subset(domain_gift_t, trait %in% significant_domain$trait) %>% 
  ...
  mutate_if(is.character, as.numeric) %>%
  rownames_to_column(., "sample") %>%
  left_join(sample_metadata[c("sample", "sex")], by = join_by(sample == sample)) %>%
  filter(!is.na(sex))
...
sample_points<- sample_points %>% filter(id!="EHI01340")
...
<!--chapter:end:10_enviromental_permanova.Rmd-->
