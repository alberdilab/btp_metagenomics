...
landcover_props <- landcover_wide %>%
  mutate(across(-id, ~ . / 100))
# Remove id column for calculations
props_matrix <- landcover_props %>% select(-id) %>% as.matrix()
```

```{r landcover_diversity}
# Calculate indices for each sample (row)
shannon <- diversity(props_matrix, index = "shannon")
simpson <- diversity(props_matrix, index = "simpson")
richness <- specnumber(props_matrix)
...
# Note: for vegan, simpson index = 1 - D, where D is Simpson's index
