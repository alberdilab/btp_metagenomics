...
Temperate_woodland_samples <- sample_metadata %>% 
  filter(broad.scale.environmental.context=="1000221 - Temperate woodland") %>% 
  dplyr::select() %>% 
  pull(sample)
...
xeric_shrubland_samples <- sample_metadata %>% 
  filter(broad.scale.environmental.context=="xeric_shrubland") %>% 
  dplyr::select(sample) %>% 
  pull()
...
mixed_forest_samples <- sample_metadata %>% 
  filter(broad.scale.environmental.context=="1000198 - Mixed forest") %>% 
  dplyr::select(sample) %>% 
  pull()
...
Cropland_samples <- sample_metadata %>% 
  filter(broad.scale.environmental.context=="1000245 - Cropland") %>% 
  dplyr::select(sample) %>% 
  pull()
