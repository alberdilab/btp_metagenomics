...
```{r exclude Sample8, message=FALSE, warning=FALSE}
...
# Recompute row-normalized counts
genome_counts_row <- genome_counts_filt %>%
  dplyr::mutate(dplyr::across(-genome, ~ . / sum(.))) %>%
  tibble::column_to_rownames("genome")

keep <- valid_samples
GIFTs_elements_community  <- GIFTs_elements_community [rownames(GIFTs_elements_community)  %in% keep, , drop = FALSE]
GIFTs_functions_community <- GIFTs_functions_community[rownames(GIFTs_functions_community) %in% keep, , drop = FALSE]
GIFTs_domains_community   <- GIFTs_domains_community  [rownames(GIFTs_domains_community)  %in% keep, , drop = FALSE]
