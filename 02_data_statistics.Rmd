...
A quick look at sampling geography (Australia/Tasmania zooms), sequencing yield, and how bases distribute across low-quality, host, unmapped, and MAG-mapped fractions. We also compare an estimated microbial fraction (SingleM) to the fraction captured by MAGs.

```{r load_data_stats, message=FALSE, warning=FALSE}
...
```{r load_map, message=FALSE, warning=FALSE}
# Load world map
...
```{r reads_stats2, message=FALSE, warning=FALSE}
sample_metadata %>% 
...
Stacked bars per sample showing GB of low-quality bases, host-mapped, unmapped, and MAG-mapped fractions.

```{r dna_fractions_stats2, message=FALSE, warning=FALSE}
...
Per sample, estimated microbial fraction (SingleM) vs recovered fraction (MAGs). Lines link the two values; points mark each estimate.
