...
alpha_long <- alpha_div %>%
  ...
  left_join(sample_metadata, by = join_by(sample == sample)) %>%
  filter(!is.na(broad_environment)) %>%             # drop NA habitat
  mutate(
    broad_environment = factor(
      broad_environment,
      levels = env_levels,
      labels = c("Mixed forest", "Temperate woodland",
                 "Xeric shrubland", "Cropland")
    )
  )

## 2) Helper for one nice panel -------------------------------------
make_alpha_plot <- function(dat, metric_name, y_lab) {
  dat %>%
    filter(metric == metric_name) %>%
    ggplot(aes(x = broad_environment,
               y = value,
...
## 3) Build the four plots ------------------------------------------
plot1 <- make_alpha_plot(alpha_long, "richness",
                         "Richness")
plot2 <- make_alpha_plot(alpha_long, "neutral",
                         "Neutral alpha diversity")
plot3 <- make_alpha_plot(alpha_long, "phylogenetic",
                         "Phylogenetic alpha diversity")
plot4 <- make_alpha_plot(alpha_long, "functional",
                         "Functional alpha diversity")
```

```{r div_plot_together, comment="", echo=FALSE,message=FALSE, warning=FALSE,fig.height=4, fig.width=16, fig.fullwidth=TRUE}

grid.arrange(plot1, plot2, plot3, plot4, ncol = 4)
# or equivalently: nrow = 1

library(gridExtra)

alpha_panel <- arrangeGrob(plot1, plot2, plot3, plot4, ncol = 4)

ggsave(
  filename = "alpha_diversity_panel.png",
  plot     = alpha_panel,
  width    = 18,   # adjust as you like
  height   = 4.5,
  units    = "in",
  dpi      = 300   # 300+ is good for printing
)
